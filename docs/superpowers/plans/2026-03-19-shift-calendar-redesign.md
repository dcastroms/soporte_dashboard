# Shift Calendar Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rediseñar la página de Gestión de Turnos: toolbar unificada arriba, barra de agentes con horas semanales, grid 24/7 a ancho completo, y diálogo multi-agente para asignar varios agentes al mismo rango de horas.

**Architecture:** Todo el estado y los controles viven dentro de `ShiftCalendar.tsx`. `shifts/page.tsx` es un shell mínimo que solo carga asignaciones iniciales. Se agrega `deleteAssignmentsForWeek` a `actions.ts` para el botón "Limpiar semana".

**Tech Stack:** Next.js 16 App Router, React 19, TypeScript, Tailwind CSS v4, Prisma + MongoDB, shadcn/ui (AlertDialog, Dialog, Button, Badge, ScrollArea), date-fns, sonner

**Spec:** `docs/superpowers/specs/2026-03-19-shift-calendar-redesign.md`
...
|------|--------|-----------------|
| `src/lib/actions.ts` | Modify | Agregar `deleteAssignmentsForWeek` |
| `src/components/dashboard/ShiftCalendar.tsx` | Rewrite | Toolbar + agent bar + grid + dialog |
| `src/app/shifts/page.tsx` | Rewrite | Shell mínimo: solo carga de datos |

---

## Task 1: Server action `deleteAssignmentsForWeek`

**Files:**
- Modify: `src/lib/actions.ts`

- [ ] **Step 1: Agregar la action al final de la sección `--- SHIFTS ---`**

Buscar `export async function deleteSupportAssignment` en `actions.ts` y agregar después del cierre de esa función:
...
- [ ] **Step 2: Verificar que `npm run lint` no da errores nuevos**

```bash
npm run lint 2>&1 | grep "actions.ts"
```

Esperado: sin errores nuevos en `actions.ts`.

- [ ] **Step 3: Commit**

```bash
git add src/lib/actions.ts
git commit -m "feat(shifts): add deleteAssignmentsForWeek server action"
```

---

## Task 2: Reescribir `ShiftCalendar.tsx`

**Files:**
- Rewrite: `src/components/dashboard/ShiftCalendar.tsx`

Este es el cambio principal. El componente pasa a gestionar su propio `currentDate` (ya no viene de `page.tsx`) y contiene todos los controles.
...
export function ShiftCalendar({ initialAssignments }: ShiftCalendarProps) {
  ...
  // Reload assignments when week changes
  useEffect(() => {
  ...
  // ── Week assignments for Google Calendar ──────────────────────────────────────
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      {/* HandoverAlert — banner condicional encima del toolbar */}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm font-bold min-w-[150px] text-center select-none">
      ...
      {Object.keys(weeklySummary).length > 0 && (
        <div className="flex items-center gap-2 flex-wrap px-3 py-2 bg-muted/40 border border-border rounded-lg">
          ...
          {Object.entries(weeklySummary)
            .sort((a, b) => b[1] - a[1])
            .map(([name, hrs]) => {
              const colorHex = getAgentColor(name);
              const colorClass = AGENT_COLOR_CLASSES[colorHex];
      ...
      <Card className="border-border overflow-hidden flex-1 flex flex-col bg-background">
        <CardContent className="p-0 flex-1 flex flex-col">
          <ScrollArea className="h-[calc(100vh-220px)]">
            <div className="min-w-[700px]" onMouseLeave={() => { if (isSelecting) handleMouseUp(); }}>
              {/* Day headers */}
      ...
      {/* Assignment Dialog */}
      <Dialog open={isDialogOpen} onOpenChange={open => { setIsDialogOpen(open); if (!open) { setSelectedAgents([]); setCustomAgent(''); } }}>
...
```bash
npm run lint 2>&1 | grep "ShiftCalendar.tsx" | head -20
```

Esperado: solo warnings (no errors) o sin output relacionado con este archivo.

- [ ] **Step 3: Commit**

```bash
git add src/components/dashboard/ShiftCalendar.tsx
git commit -m "feat(shifts): rewrite ShiftCalendar — toolbar + agent bar + multi-agent dialog"
```

---

## Task 3: Simplificar `shifts/page.tsx`

**Files:**
- Rewrite: `src/app/shifts/page.tsx`

`currentDate` y la lógica de carga ahora viven en `ShiftCalendar`. `page.tsx` solo carga los assignments iniciales de la semana actual.
...
> **Nota:** Al convertir a Server Component (sin `"use client"`), el fetch inicial ocurre en el servidor. `ShiftCalendar` recarga los datos vía `getSupportAssignments` cada vez que cambia la semana desde el cliente.

- [ ] **Step 2: Verificar lint**

```bash
npm run lint 2>&1 | grep "shifts" | head -10
```

- [ ] **Step 3: Commit**

```bash
git add src/app/shifts/page.tsx
git commit -m "refactor(shifts): page.tsx to thin server component shell"
```

---

## Task 4: Verificación manual y commit final
...
- [ ] **Step 2: Verificar en el navegador** (http://localhost:3000/shifts)

Checklist visual:
- [ ] Toolbar visible: flechas de semana + label de fechas + "Hoy" + "Limpiar semana" + Google Cal + Entrega de Turno
- [ ] Barra de agentes aparece cuando hay turnos asignados esta semana
- [ ] Chips en rojo cuando agente > 45h
- [ ] Grid muestra 24 filas uniformes (h-[40px] cada una)
- [ ] Columna de hoy resaltada con borde verde en el header
- [ ] Arrastrar verticalmente en una columna resalta celdas en verde
- [ ] Al soltar aparece el diálogo con botones de agentes
- [ ] Seleccionar múltiples agentes y hacer "Asignar" → chips aparecen en las celdas
- [ ] El ✕ en un chip elimina solo esa asignación
- [ ] "Limpiar semana" → AlertDialog de confirmación → limpia la semana
- [ ] "Hoy" navega a la semana actual
- [ ] HandoverAlert aparece solo si hay entrega pendiente (no siempre)
- [ ] Dark mode: todos los colores se ven correctamente

- [ ] **Step 3: Commit final si todo está bien**

```bash
git add -A
git commit -m "chore(shifts): final verification pass"
```
