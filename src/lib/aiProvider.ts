/**
 * AI Provider abstraction — swap between Ollama (local) and Claude API
...
const PROVIDER = (process.env.AI_PROVIDER || "ollama") as "ollama" | "claude";
...
export async function healthCheck(): Promise<{ ok: boolean; provider: string; model: string; error?: string }> {
  try {
    if (PROVIDER === "ollama") {
      ...
      // Minimal Claude check — just verify the API key is set
    ...
    return {
      ok: false,
      provider: PROVIDER,
      model: PROVIDER === "ollama" ? OLLAMA_MODEL : CLAUDE_MODEL,
